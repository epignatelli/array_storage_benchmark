...
class HDF5(TimeArrStorage):
    def save(self, arr, pth):
        with h5py.File(pth, 'w') as fh:
            fh.create_dataset("benchmark_dataset", data=arr)

    def load(self, pth):
        with h5py.File(pth, 'r') as fh:
            return fh["benchmark_dataset"][:]
...
METHODS = (NPY, HDF5)  # todo: Stata
# METHODS = (Csv, CsvGzip, JSON, JSONGzip, b64Enc, Pickle, PickleGzip, Binary, BinaryGzip, NPY, NPYCompr, PNG, MatFile, HDF5)  # todo: Stata
# METHODS = (HDF5, )  # todo: Stata
# html
# excel
# stata
